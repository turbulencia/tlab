...
module SpectraMod
    ...
    public :: REDUCE_SPECTRUM, REDUCE_CORRELATION
    public :: WRITE_SPECTRUM1D
    public :: RADIAL_SAMPLESIZE
#ifdef USE_MPI
...
! nyquist frequency and zero frequency account different for the variance
        ...
        complex(wp), pointer :: c_in(:, :) => null(), c_tmp1(:, :) => null()
...
! add variance from nyquist frequency for the Parseval identity to hold
                ...
                if (ims_pro_i == ims_npro_i - 1) then
                    ...
                    variance(y, 1) = variance(y, 1) + real(c_in(ip, kz))
#ifdef USE_MPI
                end if
#endif
