...
! Finalize Oz spectrum
    ...
    if (ims_npro_k > 1) then
        ...
        call TLabMPI_TransposeK_Forward(tmp_z(:, :, 2), wrk2d(:, :, 1), ims_trp_plan_k(TLAB_MPI_TRP_K_AUX2))
...
! drop the Nyquist frequency; we add it to the previous mode to keep structure below
    ...
    if (ims_npro_k > 1) then
        ...
        call TLabMPI_TransposeK_Backward(wrk2d(:, :, 1), tmp_z(:, :, 1), ims_trp_plan_k(TLAB_MPI_TRP_K_AUX2))
