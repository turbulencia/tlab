...
!  ..scalar arguments..
      TINTEGER mu,mv,mz,nuest,nvest,maxit,nc,nu,nv,lastdi,nplusu,&
           nplusv,lasttu,lwrk,ier
      TREAL z0,r,s,tol,fp,fp0,fpold,reducu,reducv,step
!  ..array arguments..
      TINTEGER iopt(3),ider(2),nrdatu(nuest),nrdatv(nvest),nru(mu),&
       nrv(mv)
      TREAL u(mu),v(mv),z(mz),tu(nuest),tv(nvest),c(nc),fpintu(nuest),&
       fpintv(nvest),dz(3),wrk(lwrk)
!  ..local scalars..
      TREAL acc,fpms,f1,f2,f3,p,per,pi,p1,p2,p3,vb,ve,zmax,zmin,rn,&
           one,con1,con4,con9
      TINTEGER i,ich1,ich3,ifbu,ifbv,ifsu,ifsv,istart,iter,i1,i2,j,ju,&
