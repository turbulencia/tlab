...
#include "mpif.h"
  ...
  IF ( ims_nb_thrsupp_provided < MPI_THREAD_SERIALIZED ) THEN
    CALL IO_WRITE_ASCII(efile,"DNS_START. MPI Library is missing the needed level of thread support for nonblocking communication.")
    CALL IO_WRITE_ASCII(efile,"DNS_START. Try MPI_THREAD_FUNNELED after reading the documentation.")
  ...
  IF ( ims_npro .EQ. 0 ) THEN
    CALL IO_WRITE_ASCII(efile, 'DNS_START. Number of processors is zero.')
