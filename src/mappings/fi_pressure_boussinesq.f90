...
! Calculate forcing term Oz
  ...
  ENDIF
  IF (istagger .EQ. 1 ) THEN
     CALL OPR_PARTIAL_Z(OPR_P1_INT_VP, imax,jmax,kmax, bcs, g(3), tmp5,tmp2, wrk3d, wrk2d,wrk3d)
...
! #######################################################################
  nxy  = imax*jmax; ip_b = 1; ip_t = imax*(jmax-1) + 1
  DO k = 1,kmax
    wrk2d(1:imax,k,1) = tmp4(ip_b:ip_b+imax-1)
    wrk2d(1:imax,k,2) = tmp4(ip_t:ip_t+imax-1)
    ip_t = ip_t + nxy ! top
    ip_b = ip_b + nxy ! bottom
  ENDDO
