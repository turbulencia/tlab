...
module Radiation
    ...
    integer :: nbcs                                     ! number of boundary conditions at domain top
...
!########################################################################
    subroutine Radiation_Initialize(inifile)
        ...
        if (infraredProps%type /= TYPE_NONE) then
            ...
            nbcs = idummy - 1                               ! number of boundary conditions at domain top
        ...
        if (infraredProps%type == TYPE_BULK1DLOCAL) then             ! backwards compatibility
            ...
            infraredProps%parameters(1) = 1.0_wp/infraredProps%parameters(2)
        end if
    ...
    subroutine Radiation_Infrared_Y(localProps, nx, ny, nz, g, s, source, b, tmp1, tmp2, flux)
        ...
        case (TYPE_IR_BAND)
            ...
            do iband = 1, nbands
                ...
                if (nbcs == 1) then                                     ! set boundary conditions
                    bcs_ht(1:nxz) = localProps%auxiliar(1)*(beta(1, iband) + t_ht*(beta(2, iband) + t_ht*beta(3, iband)))   ! downward flux at domain top
                else
