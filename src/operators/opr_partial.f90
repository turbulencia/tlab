...
SUBROUTINE OPR_PARTIAL0_INT(dir, nlines, bcs, g, u,result, wrk2d)
...
! pure interpolation from one grid to another   
  IF ( dir .EQ. 0 ) THEN ! vp
    ...
    ELSE
      SELECT CASE( g%mode_fdm )
      CASE( FDM_COM4_JACOBIAN, FDM_COM6_JACOBIAN, FDM_COM6_DIRECT, FDM_COM8_JACOBIAN )
        CALL TLAB_WRITE_ASCII(efile, 'OPR_PARTIAL0_INT. Not implemented for non-periodic Bcs.')
        CALL TLAB_STOP(DNS_ERROR_NOTIMPL)    
      END SELECT
      ! ip = .... ! CALL TRIDSS(g%size,nlines, g%lu0i(1,ip+1),g%lu0i(1,ip+2),g%lu0i(1,ip+3), result)
    ENDIF
  ! =====================================================================
  ELSE IF ( dir .EQ. 1 ) THEN ! pv
    IF ( g%periodic ) THEN
      SELECT CASE( g%mode_fdm )        
      CASE( FDM_COM4_JACOBIAN, FDM_COM6_JACOBIAN, FDM_COM6_DIRECT, FDM_COM8_JACOBIAN )
...
SUBROUTINE OPR_PARTIAL1_INT(dir, nlines, bcs, g, u,result, wrk2d)
...
! 1st interpolatory derivative from one grid to another   
  IF ( dir .EQ. 0 ) THEN ! vp
    ...
    ELSE
      SELECT CASE( g%mode_fdm )
      CASE( FDM_COM4_JACOBIAN, FDM_COM6_JACOBIAN, FDM_COM6_DIRECT, FDM_COM8_JACOBIAN )
        CALL TLAB_WRITE_ASCII(efile, 'OPR_PARTIAL1_INT. Not implemented for non-periodic Bcs.')
        CALL TLAB_STOP(DNS_ERROR_NOTIMPL)    
      END SELECT
      ! ip = .... ! CALL TRIDSS(g%size,nlines, g%lu1i(1,ip+1),g%lu1i(1,ip+2),g%lu1i(1,ip+3), result)
    ENDIF
  ! =====================================================================
  ELSE IF ( dir .EQ. 1 ) THEN ! pv
    IF ( g%periodic ) THEN
...
! Check whether we need to calculate the 1. order derivative
  ...
  CASE( OPR_P0_INT_VP )
     CALL OPR_PARTIAL0_INT(i0, nyz, bcs, g, p_b,p_c, wrk2d)

  CASE( OPR_P0_INT_PV )
     CALL OPR_PARTIAL0_INT(i1, nyz, bcs, g, p_b,p_c, wrk2d)

  CASE( OPR_P1_INT_VP )
     CALL OPR_PARTIAL1_INT(i0, nyz, bcs, g, p_b,p_c, wrk2d)

  CASE( OPR_P1_INT_PV )
     CALL OPR_PARTIAL1_INT(i1, nyz, bcs, g, p_b,p_c, wrk2d)
...
! Check whether we need to calculate the 1. order derivative
  ...
  CASE( OPR_P0_INT_VP )
     CALL OPR_PARTIAL0_INT(i0, nxy, bcs, g, p_a,p_b, wrk2d)
 
  CASE( OPR_P0_INT_PV )
     CALL OPR_PARTIAL0_INT(i1, nxy, bcs, g, p_a,p_b, wrk2d)

  CASE( OPR_P1_INT_VP )
     CALL OPR_PARTIAL1_INT(i0, nxy, bcs, g, p_a,p_b, wrk2d)

  CASE( OPR_P1_INT_PV )
     CALL OPR_PARTIAL1_INT(i1, nxy, bcs, g, p_a,p_b, wrk2d)
