...
!# DESCRIPTION
!#
!# Solve Lap p - \alpha p = a using Fourier in xOz planes, to rewritte the problem
!# as 
!#     \hat{p}''-(\lambda+\alpha) \hat{p} = \hat{a}
!#
!# where \lambda = kx^2+kz^2
!#
!# Routine derived from POISSON
!#
!########################################################################
!# ARGUMENTS 
!#
...
SUBROUTINE OPR_HELMHOLTZ_FXZ_2_NEW(nx,ny,nz, nfield, ibc, alpha, dx,dy,dz,&
     a, tmp1,tmp2, bcs_hb,bcs_ht, aux, wrk1d,wrk3d)

  USE DNS_TYPES,  ONLY : pointers_structure
  USE DNS_GLOBAL, ONLY : isize_txc_dimz, imax_total,jmax_total,kmax_total, inb_grid_2,imode_fdm
...
! ################################################################### 
  ...
  ENDDO
  RETURN
END SUBROUTINE OPR_HELMHOLTZ_FXZ_2_NEW
