...
!#
!########################################################################
SUBROUTINE  RHS_FLOW_GLOBAL_INCOMPRESSIBLE_4&
...
! #######################################################################
  ...
  CALL PARTIAL_YY(i1, iunify, imode_fdm, imax,jmax,kmax, j1bc,&
       dy, w, tmp3, i0,i0, i0,i0, tmp2, wrk1d,wrk2d,wrk3d)
  CALL PARTIAL_XX(i1, iunifx, imode_fdm, imax,jmax,kmax, i1bc,&
       dx, w, tmp5, i0,i0, i0,i0, tmp4, wrk1d,wrk2d,wrk3d)
...
!$omp parallel default( shared ) private( ij, dummy,srt,end,siz )
  ...
  IF ( icoriolis .EQ. EQNS_COR_NORMALIZED ) THEN
     dummy = rotn_vector(2)
     DO ij = srt,end
        h3(ij) = h3(ij) + dummy*( u(ij)-u_geo ) + tmp1(ij) + visc*( tmp5(ij)+tmp3(ij) ) &
             - ( u(ij)*tmp4(ij) + v(ij)*tmp2(ij) )
     ENDDO
! -----------------------------------------------------------------------
  ELSE
     DO ij = srt,end
        h3(ij) = h3(ij) + tmp1(ij) + visc*( tmp5(ij)+tmp3(ij) ) &
             - ( u(ij)*tmp4(ij) + v(ij)*tmp2(ij) )
     ENDDO
  ENDIF
!$omp end parallel
...
! #######################################################################
  CALL OPR_BURGERS_X(i0,i0, iunifx, imode_fdm, imax,jmax,kmax, i1bc,&
       dx, u,u,u, tmp1, i0,i0, i0,i0, tmp2, wrk2d,wrk3d)
  CALL PARTIAL_YY(i1, iunify, imode_fdm, imax,jmax,kmax, j1bc,&
       dy, u, tmp3, i0,i0, i0,i0, tmp2, wrk1d,wrk2d,wrk3d)
  CALL OPR_BURGERS_Z(i1,i0, iunifz, imode_fdm, imax,jmax,kmax, k1bc,&
       dz, u,w,tmp6, tmp5, i0,i0, i0,i0, tmp4, wrk2d,wrk3d) ! tmp6 contains w transposed
...
!$omp parallel default( shared ) private( ij, dummy,srt,end,siz )
  ...
  IF ( icoriolis .EQ. EQNS_COR_NORMALIZED ) THEN
     dummy = rotn_vector(2)
     DO ij = srt, end 
        h1(ij) = h1(ij) + dummy*( w_geo-w(ij) ) + tmp1(ij) + tmp5(ij) + visc*tmp3(ij) -  v(ij)*tmp2(ij)
     ENDDO
! -----------------------------------------------------------------------
  ELSE
     DO ij = srt,end
        h1(ij) = h1(ij) + tmp1(ij) + tmp5(ij) + visc*tmp3(ij) -  v(ij)*tmp2(ij)
     ENDDO
  ENDIF
!$omp end parallel
...
! #######################################################################
  ...
  CALL PARTIAL_XX(i1, iunifx, imode_fdm, imax,jmax,kmax, i1bc,&
       dx, v, tmp3, i0,i0, i0,i0, tmp2, wrk1d,wrk2d,wrk3d)
  CALL OPR_BURGERS_Z(i1,i0, iunifz, imode_fdm, imax,jmax,kmax, k1bc,&
       dz, v,w,tmp6, tmp5, i0,i0, i0,i0, tmp4, wrk2d,wrk3d) ! tmp6 contains w transposed
...
!$omp private( ij,   srt,end,siz, dummy )
     ...
     CALL DNS_OMP_PARTITION(isize_field, srt,end,siz) 
     DO ij = srt,end
        h2(ij) = h2(ij) + tmp1(ij) + tmp5(ij) + visc*tmp3(ij) - u(ij)*tmp2(ij)
     ENDDO
!$omp end parallel 
! -----------------------------------------------------------------------
  ELSE
...
!$omp private( ij,   srt,end,siz, dummy )
     ...
     dummy = body_vector(2)
     DO ij = srt,end
        h2(ij) = h2(ij) + dummy*wrk3d(ij) + tmp1(ij) + tmp5(ij)+ visc*tmp3(ij) - u(ij)*tmp2(ij)
     ENDDO
!$omp end parallel 
  ENDIF
...
! -----------------------------------------------------------------------
  ...
  RETURN
END SUBROUTINE RHS_FLOW_GLOBAL_INCOMPRESSIBLE_4
