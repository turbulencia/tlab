...
! #######################################################################
  IF (damkohler(1) .GT. C_0_R) THEN !It supersatturation is allowed
     ...
     CALL THERMO_AIRWATER_QSAT(imax,jmax,kmax, s(:,2:3),al_h, pbackground, tmp7, tmp4) !Temperature in tmp7 and saturation concentration in tmp4 
...
!Put temperature in tmp7
        CALL THERMO_AIRWATER_TEMPERATURE(imax, jmax, kmax, s(1,2), s(1,1), tmp7) 
     ENDIF
  ENDIF
