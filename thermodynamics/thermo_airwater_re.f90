...
! initialize homogeneous data
  ...
  HEAT_CAPACITY_LV = THERMO_AI(1,1,3)-THERMO_AI(1,1,1)+GRATIO*WGHT_INV(1) 
  HEAT_CAPACITY_LD = THERMO_AI(1,1,3)-THERMO_AI(1,1,2)+GRATIO*WGHT_INV(2) 
...
! change T consistently
           T(i) = ( e(i) - &
                ...
                z1(i,2)*LATENT_HEAT )/&
                ( z1(i,1)*HEAT_CAPACITY_VD + THERMO_AI(1,1,2)-GRATIO*WGHT_INV(2) &
...
! if q_s < q_t, then we have to repeat calculation of T
     ELSE
        B_LOC(2) = B_LOC_CONST_2 + rho(i)*WGHT_INV(1)*&
             ( e(i)-z1(i,1)*(THERMO_AI(6,1,3)-THERMO_AI(6,1,2))-THERMO_AI(6,1,2) )
        B_LOC(3) = B_LOC_CONST_3 - rho(i)*WGHT_INV(1)*&
             ( z1(i,1)*HEAT_CAPACITY_LD + THERMO_AI(1,1,2)-GRATIO*WGHT_INV(2) )
! IF ( dsmooth .GT. C_0_R ) THEN
! dsmooth_loc = dsmooth*qsat
