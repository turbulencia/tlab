...
!#######################################################################
!#######################################################################
SUBROUTINE RHS_PARTICLE_GLOBAL(q,s, wrk1d,wrk2d,wrk3d,txc,l_q,l_hq, l_tags,l_comm)
...
! Setting pointers
  nfield = 0
  nfield = nfield+1; data(nfield)%field => q(:,1)
  nfield = nfield+1; data(nfield)%field => q(:,2)
  nfield = nfield+1; data(nfield)%field => q(:,3)
...
! Setting pointers
     nfield = nfield+1; data(nfield)%field => txc(:,1)
     nfield = nfield+1; data(nfield)%field => txc(:,2)
     nfield = nfield+1; data(nfield)%field => txc(:,3)
     nfield = nfield+1; data(nfield)%field => txc(:,4)
...
! ######################################################################
  ...
  CALL PARTICLE_HALO_K(nfield, data, halo_field_2, wrk3d(1), wrk3d(imax*jmax*inb_lag_total_interp+1), &
       wrk2d(1), wrk2d(2*(jmax*inb_lag_total_interp+1)))
  ...
  CALL PARTICLE_HALO_I(nfield, data, halo_field_1, halo_field_3, wrk3d(1), wrk3d(jmax*(kmax+1)*inb_lag_total_interp+1),&
       wrk2d(1), wrk2d(jmax*inb_lag_total_interp+1), wrk2d(2*(jmax*inb_lag_total_interp+1)))
#else
  CALL PARTICLE_HALO_SERIAL(nfield, data, halo_field_1, halo_field_2, halo_field_3 )
