...
!#
!########################################################################
SUBROUTINE PARTICLE_SEND_RECV(nzone_grid, nzone_west, nzone_east,nzone_south, nzone_north,x_or_z,&
...
#include "mpif.h"
  ...
  TINTEGER source_west, source_east, source_south, source_north
  ...
  TINTEGER x_or_z
  TINTEGER nzone_grid, nzone_west, nzone_east, nzone_send_west, nzone_send_east
  TINTEGER nzone_south, nzone_north, nzone_send_south, nzone_send_north
  ...
  IF (x_or_z .EQ. 1) THEN !Sending East-West
    ...
    !###################################################################
    !X-DIR
    !Setting up destination and source + send/recv number of particles
    !which will be send
    !###################################################################
...
#endif
  RETURN
END SUBROUTINE PARTICLE_SEND_RECV
