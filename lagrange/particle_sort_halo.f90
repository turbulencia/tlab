...
!#
!########################################################################
SUBROUTINE PARTICLE_SORT_HALO(nzone_grid, halo_zone_x, halo_zone_z, halo_zone_diagonal, l_hq, l_tags, l_q)
...
#ifdef USE_MPI
  right_limit=g(1)%nodes(imax*(ims_pro_i+1))  ! right_limit is east
  upper_limit=g(3)%nodes(kmax*(ims_pro_k+1))  !upper_limit is north
#else
  right_limit=g(1)%nodes(g(1)%size)  ! right_limit is east
