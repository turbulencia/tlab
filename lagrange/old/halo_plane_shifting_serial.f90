...
#include "integers.h"
  ...
  DO i=1,3 
    halo_field_1(1,1:jmax,1:kmax,i)=q(g(1)%size,1:jmax,1:kmax,i)  
    halo_field_1(2,1:jmax,1:kmax,i)=q(1,1:jmax,1:kmax,i)  

    halo_field_2(1:imax,1:jmax,1,i)=q(1:imax,1:jmax,g(3)%size,i)
    halo_field_2(1:imax,1:jmax,2,i)=q(1:imax,1:jmax,1,i)
    
    halo_field_3(1,1:jmax,1,i)=q(g(1)%size,1:jmax,g(3)%size,i)
    halo_field_3(1,1:jmax,2,i)=q(g(1)%size,1:jmax,1,i)
    halo_field_3(2,1:jmax,1,i)=q(1,1:jmax,g(3)%size,i)
    halo_field_3(2,1:jmax,2,i)=q(1,1:jmax,1,i)
  END DO

  DO i=4,inb_lag_total_interp
    iloc=i - 3
    halo_field_1(1,1:jmax,1:kmax,i)=txc(g(1)%size,1:jmax,1:kmax,iloc)  
    halo_field_1(2,1:jmax,1:kmax,i)=txc(1,1:jmax,1:kmax,iloc)  

    halo_field_2(1:imax,1:jmax,1,i)=txc(1:imax,1:jmax,g(3)%size,iloc)
    halo_field_2(1:imax,1:jmax,2,i)=txc(1:imax,1:jmax,1,iloc)
    
    halo_field_3(1,1:jmax,1,i)=txc(g(1)%size,1:jmax,g(3)%size,iloc)
    halo_field_3(1,1:jmax,2,i)=txc(g(1)%size,1:jmax,1,iloc)
    halo_field_3(2,1:jmax,1,i)=txc(1,1:jmax,g(3)%size,iloc)
