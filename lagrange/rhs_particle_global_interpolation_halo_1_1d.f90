...
#include "integers.h"
  ...
  TREAL, DIMENSION(*)                 :: y
  TREAL, DIMENSION(2,jmax,kmax)       :: halo_field
  ...
  ELSE !2D CASE
    DO i=halo_start,halo_end
      ...
      !#######################################################################
       ...
       +cube_g_p(4)*halo_field(gridpoint(1),gridpoint(4),gridpoint(5)) &
       +cube_g_p(1)*halo_field(gridpoint(2),gridpoint(4),gridpoint(5)) &
       +cube_g_p(2)*halo_field(gridpoint(2),gridpoint(3),gridpoint(5))) 
  ...
  RETURN
END SUBROUTINE RHS_PARTICLE_GLOBAL_INTERPOLATION_HALO_1_1D
