...
#include "mpif.h"
  ...
  REAL(8), DIMENSION(:),   ALLOCATABLE :: wrk3d
...
! -------------------------------------------------------------------
     IF ( ims_npro_i .GT. 1 ) THEN
        CALL DNS_MPI_TRPF_I(a(1,1), wrk3d, ims_ds_i, ims_dr_i, ims_size_i)
        ! CALL MPI_ALLTOALLV(a(1,1), ims_size_i, ims_ds_i, MPI_REAL8, &
        !                    wrk3d,  ims_size_i, ims_dr_i, MPI_REAL8, ims_comm_x)
...
! -------------------------------------------------------------------
     IF ( ims_npro_k .GT. 1 ) THEN
        CALL DNS_MPI_TRPF_K(a(1,1), wrk3d, ims_ds_k, ims_dr_k, ims_size_k)
        ! CALL MPI_ALLTOALLV(a(1,1), ims_size_k, ims_ds_k, MPI_REAL8, &
        !                    wrk3d,  ims_size_k, ims_dr_k, MPI_REAL8, ims_comm_z)
...
#include "mpif.h"
  ...
  INTEGER(4), DIMENSION(ims_npro_k), INTENT(IN)  :: dsend, drecv ! displacements
  INTEGER(4), DIMENSION(ims_npro_k), INTENT(IN)  :: size
...
! #######################################################################
  ip = ims_pro_k; n = ip + 1
  CALL MPI_ISEND(a(dsend(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_z, mpireq(1), ims_err)  
  CALL MPI_IRECV(b(drecv(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_z, mpireq(2), ims_err)
...
! #######################################################################
  ...
  DO n = 1,ims_npro_k
     ...
     IF ( ip .NE. ims_pro_k ) THEN
        l = l + 1      
        CALL MPI_ISEND(a(dsend(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_z, mpireq(l), ims_err)
        l = l + 1
        CALL MPI_IRECV(b(drecv(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_z, mpireq(l), ims_err)        
     ENDIF
  ENDDO
...
#include "mpif.h"
  ...
  REAL(8),    DIMENSION(*),          INTENT(IN)  :: a
  REAL(8),    DIMENSION(*),          INTENT(OUT) :: b
  INTEGER(4), DIMENSION(ims_npro_i), INTENT(IN)  :: dsend, drecv ! displacements
  INTEGER(4), DIMENSION(ims_npro_i), INTENT(IN)  :: size
...
! #######################################################################
  ip = ims_pro_i; n = ip + 1
  CALL MPI_ISEND(a(dsend(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_x, mpireq(1), ims_err)  
  CALL MPI_IRECV(b(drecv(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_x, mpireq(2), ims_err)
...
! #######################################################################
  ...
  DO n = 1,ims_npro_i
     ...
     IF ( ip .NE. ims_pro_i ) THEN
        l = l + 1
        CALL MPI_ISEND(a(dsend(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_x, mpireq(l), ims_err)
        l = l + 1 
        CALL MPI_IRECV(b(drecv(n)+1), size(n), MPI_REAL8, ip, ims_tag, ims_comm_x, mpireq(l), ims_err)
     ENDIF
  ENDDO
