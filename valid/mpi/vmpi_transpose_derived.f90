...
PROGRAM VMPI
  ...
  INTEGER(4), PARAMETER :: nmax = 10000 ! number of repetitions of operations
...
! transposition along X
  ...
  INTEGER,  DIMENSION(:), ALLOCATABLE :: ims_ts_i, ims_tr_i ! mpi derived types
...
! transposition along Z
  ...
  INTEGER,  DIMENSION(:), ALLOCATABLE :: ims_ts_k, ims_tr_k ! mpi derived types
...
! Initialize parallel part
  ...
  ! Communication along I
  ALLOCATE(ims_map_i(ims_npro_i))
  ALLOCATE(ims_size_i(ims_npro_i)) ! size and displacements
  ALLOCATE(ims_ds_i(ims_npro_i))
  ALLOCATE(ims_dr_i(ims_npro_i))
  ALLOCATE(ims_ts_i(ims_npro_i)) ! mpi derived types
  ALLOCATE(ims_tr_i(ims_npro_i))

  ! Communication along K
  ALLOCATE(ims_map_k(ims_npro_k))
  ALLOCATE(ims_size_k(ims_npro_k)) ! size and displacements
  ALLOCATE(ims_ds_k(ims_npro_k))
  ALLOCATE(ims_dr_k(ims_npro_k))
  ALLOCATE(ims_ts_k(ims_npro_k)) ! mpi derived types
  ALLOCATE(ims_tr_k(ims_npro_k))
...
! #######################################################################
  IF ( ims_npro_i .GT. 1 ) THEN
     ! Calculate size
     ims_size_i(:) = jmax*kmax /ims_npro_i
     ! Calculate Displacements in Forward Send/Receive
     ims_ds_i(1) = 0
     ims_dr_i(1) = 0
     DO ip = 2,ims_npro_i
        ims_ds_i(ip) = ims_ds_i(ip-1) + imax *ims_size_i(ip-1)
        ims_dr_i(ip) = ims_dr_i(ip-1) + imax
     ENDDO
     ! Define derived types
     DO ip = 1,ims_npro_i
        CALL MPI_TYPE_VECTOR(ims_size_i(ip), imax, imax,             MPI_REAL8, ims_ts_i(ip), ims_err)
        CALL MPI_TYPE_COMMIT(ims_ts_i(ip), ims_err)
        CALL MPI_TYPE_VECTOR(ims_size_i(ip), imax, imax *ims_npro_i, MPI_REAL8, ims_tr_i(ip), ims_err)
        CALL MPI_TYPE_COMMIT(ims_tr_i(ip), ims_err)
     ENDDO
  ENDIF
  
  IF ( ims_npro_k .GT. 1 ) THEN
     ! Calculate size
     ims_size_k(:) = imax*jmax /ims_npro_k
     ! Calculate Displacements in Forward Send/Receive
     ims_ds_k(1) = 0
     ims_dr_k(1) = 0
     DO ip = 2,ims_npro_k
        ims_ds_k(ip) = ims_ds_k(ip-1) + ims_size_k(ip-1)
        ims_dr_k(ip) = ims_dr_k(ip-1) + ims_size_k(ip-1) *kmax
     ENDDO
     ! Define derived types
     DO ip = 1,ims_npro_k
        CALL MPI_TYPE_VECTOR(kmax, ims_size_k(ip), imax*jmax,      MPI_REAL8, ims_ts_k(ip), ims_err)
        CALL MPI_TYPE_COMMIT(ims_ts_k(ip), ims_err)
        CALL MPI_TYPE_VECTOR(kmax, ims_size_k(ip), ims_size_k(ip), MPI_REAL8, ims_tr_k(ip), ims_err)
        CALL MPI_TYPE_COMMIT(ims_tr_k(ip), ims_err)
     ENDDO
  ENDIF
...
! -------------------------------------------------------------------
     IF ( ims_npro_i .GT. 1 ) THEN
        ...
        ! Same processor
        ip = ims_pro_i; n = ip + 1
        CALL MPI_ISEND(a(ims_ds_i(n)+1,1), 1, ims_ts_i(n), ip, 1,           ims_comm_x, mpireq(1), ims_err)  
        CALL MPI_IRECV(b(ims_dr_i(n)+1),   1, ims_tr_i(n), ip, MPI_ANY_TAG, ims_comm_x, mpireq(2), ims_err)
        ...
        DO n = 1,ims_npro_i
           ...
           IF ( ip .NE. ims_pro_i ) THEN
              l = l + 1
              CALL MPI_ISEND(a(ims_ds_i(n)+1,1), 1, ims_ts_i(n), ip, 1,           ims_comm_x, mpireq(l), ims_err)
              l = l + 1 
              CALL MPI_IRECV(b(ims_dr_i(n)+1),   1, ims_tr_i(n), ip, MPI_ANY_TAG, ims_comm_x, mpireq(l), ims_err)
           ENDIF
        ENDDO
        ...
        ! Same processor
        ip = ims_pro_i; n = ip + 1
        CALL MPI_ISEND(b(ims_dr_i(n)+1),   1, ims_tr_i(n), ip, 1,           ims_comm_x, mpireq(1), ims_err)  
        CALL MPI_IRECV(a(ims_ds_i(n)+1,2), 1, ims_ts_i(n), ip, MPI_ANY_TAG, ims_comm_x, mpireq(2), ims_err)
        CALL MPI_WAITALL(2, mpireq, status, ims_err)
        ...
        DO n = 1,ims_npro_i
           ...
           IF ( ip .NE. ims_pro_i ) THEN
              l = l + 1
              CALL MPI_ISEND(b(ims_dr_i(n)+1),   1, ims_tr_i(n), ip, 1,           ims_comm_x, mpireq(l), ims_err)
              l = l + 1 
              CALL MPI_IRECV(a(ims_ds_i(n)+1,2), 1, ims_ts_i(n), ip, MPI_ANY_TAG, ims_comm_x, mpireq(l), ims_err)
           ENDIF
        ENDDO
        CALL MPI_WAITALL(ims_npro_i*2-2, mpireq(3:), status(:,3:), ims_err)
...
!         ! Same processor
!         ip = ims_pro_k; n = ip + 1
!         CALL MPI_ISEND(a(ims_ds_k(n)+1,1), 1, ims_ts_k(n), ip, 1,           ims_comm_z, mpireq(1), ims_err)  
!         CALL MPI_IRECV(b(ims_dr_k(n)+1),   1, ims_tr_k(n), ip, MPI_ANY_TAG, ims_comm_z, mpireq(2), ims_err)
!         CALL MPI_WAITALL(2, mpireq, status, ims_err)
...
!               l = l + 1      
!               CALL MPI_ISEND(a(ims_ds_k(n)+1,1), 1, ims_ts_k(n), ip, 1,           ims_comm_z, mpireq(l), ims_err)
!               l = l + 1
!               CALL MPI_IRECV(b(ims_dr_k(n)+1),   1, ims_tr_k(n), ip, MPI_ANY_TAG, ims_comm_z, mpireq(l), ims_err)        
!            ENDIF
!         ENDDO
...
!         ! Same processor
!         ip = ims_pro_k; n = ip + 1
!         CALL MPI_ISEND(b(ims_dr_k(n)+1),   1, ims_tr_k(n), ip, 1,           ims_comm_z, mpireq(1), ims_err)  
!         CALL MPI_IRECV(a(ims_ds_k(n)+1,2), 1, ims_ts_k(n), ip, MPI_ANY_TAG, ims_comm_z, mpireq(2), ims_err)
!         CALL MPI_WAITALL(2, mpireq, status, ims_err)
...
!            IF ( ip .NE. ims_pro_k ) THEN
!               l = l + 1      
!               CALL MPI_ISEND(b(ims_dr_k(n)+1),   1, ims_tr_k(n), ip, 1,           ims_comm_z, mpireq(l), ims_err)
!               l = l + 1
!               CALL MPI_IRECV(a(ims_ds_k(n)+1,2), 1, ims_ts_k(n), ip, MPI_ANY_TAG, ims_comm_z, mpireq(l), ims_err)        
!            ENDIF
!         ENDDO
!         CALL MPI_WAITALL(ims_npro_k*2-2, mpireq(3:), status(:,3:), ims_err)
