...
#include "integers.h"
  ...
  PARAMETER(imax=3200, len=1, inb_grid=3+4*3+4*3+1*5)
  TREAL lambda, x_0
...
! single-mode
        ! u(l,i)     =                                 &
        !       SIN(C_2_R*C_PI_R/g%scale*lambda*x(i)+C_PI_R/C_4_R)
        ! du1_a(l,i) = (C_2_R*C_PI_R/g%scale*lambda)    &
        !      *COS(C_2_R*C_PI_R/g%scale*lambda*x(i)+C_PI_R/C_4_R)
...
! ###################################################################
  ...
  DO i = 1,imax
     DO l = 1,len
        ...
        WRITE(20,1000) g%nodes(i), u(l,i), du2_a(l,i), du2_n1(l,i), du2_a(l,i)-du2_n1(l,i)
