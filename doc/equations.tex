...
\begin{subequations}\label{equ:radflux}
    \begin{align}
        &F_\uparrow(z_{i+1}) = e^{-\tau(z_i,z_{i+1})/\mu}\left[ F_\uparrow(z_i) + 2\pi\mu\int_{z_i}^{z_{i+1}} a B e^{\tau(z_i,z')/\mu}\mathrm{d}z'\right]\notag  \;,\\
        &F_\downarrow(z_i) = e^{-\tau(z_i,z_{i+1})/\mu}\left[ F_\downarrow(z_{i+1}) + 2\pi\mu\int_{z_i}^{z_{i+1}} a B e^{\tau(z',z_{i+1})/\mu}\mathrm{d}z'\right]\notag  \;,\\
    \end{align}
\end{subequations}
where $\{z_i\}_{i=1}^N$ is the set of grid points in which the interval $(z_\mathrm{min}, z_\mathrm{max})$ is discretized.
