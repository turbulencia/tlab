...
A second model considers liquid water and vapor and calculates $F_{\uparrow,z_\mathrm{min}}$ from the surface balance equation
\begin{equation}
...
We partly retain the frequency dependence by considering frequency bands where the absorption coefficient is constant. These bands might be different for different components and we solve problem~(\ref{equ:rte}) separately for different bands and components:
\begin{equation}
    \partial_s F_\mathrm{b}+(a_\mathrm{b}/\bar{\mu}) F_\mathrm{b} = (a_\mathrm{b}/\bar{\mu}) B_\mathrm{b}\;,\qquad F_\mathrm{b}|_{s=0}=F_0 \;,
\end{equation}
where the subscript $\mathrm{b}$ indicates band.

We can use the same solvers as for the problem~(\ref{equ:rte}) but we need the function
\begin{equation}
    B_\mathrm{b}(\nu_1,\nu_2,T)\equiv\int_{\nu_1}^{\nu_2}B_\nu(\nu,T)\mathrm{d} \nu\;,
\end{equation}
...
\begin{equation}
    \int_0^\infty B_\nu(\nu,T)\mathrm{d}\nu = B(T)\;,
\end{equation}
where $B(T)\equiv\sigma T^4$ has been defined in~(\ref{equ:sb}).

The integral over a finite interval of frequencies can be written as
\begin{equation}
    B_\mathrm{b}(\nu_1,\nu_2,T)=\beta_\mathrm{b}(T)B(T) \;,\qquad
    \beta_\mathrm{b}(T)\equiv\frac{15}{\pi^4}\int_{x_1(T)}^{x_2(T)}\frac{x^3}{e^x-1}\mathrm{d}x \;,
\end{equation}
and the limits of integration are $x_i(t)\equiv c_2\nu_i/T$. For a given band $(\nu_1,\,\nu_2)$, we can precalculate numerically $\beta_\mathrm{b}(T)$ as a function of $T$ for the interval of temperatures of interest and use a fit to $\beta_\mathrm{b}(T)$ in that interval to calculate the term during the simulation. The radiative transfer problem to solve is then
\begin{equation}
    \partial_s F_\mathrm{b}+(a_\mathrm{b}/\bar{\mu}) F_\mathrm{b} = (a_\mathrm{b}/\bar{\mu}) B_\mathrm{b}\;,\qquad F_\mathrm{b}|_{s=0}=F_0 \;,
\end{equation}
where $a_\mathrm{b}=\kappa_{\mathrm{b},i} \rho \zeta_i$ and $\kappa_{\mathrm{b},i}$ is the constant value of the mass absorption coefficient in band b.
...
\end{equation}
In this way, we mimic the polynomial fits to saturation pressure and heat capacities that are customary in numerical simulations. The coefficients for the 2 vapor bands are given in Table~\ref{tab:radiation}.
