...
Practically, the implementation is completely embedded in the routine {\tt operators/opr\_radiation} and uses data provided in the block \texttt{[Radiation]} (i.e., assumes $\mathrm{\bf Da}_\mathrm{R}=1$ and all coefficients enter in the definition of $\omega_\mathrm{R}$). One formulation is a one-dimensional bulk model, where
\begin{equation}
    ...
    \right)\;.
\end{equation}
The scalar indeces $\{\beta\,,\gamma\}$ and the parameters $\{\alpha_0\,,\alpha_1\,,\alpha_2\}$ need to be provided. Default values are $\beta=1$, $\gamma=${\tt inb\_scal\_array} and $\alpha_2=0$. The scalar field $ \zeta_{\gamma}$ can be an average profile, instead of instantaneous values, and non-linear mapping functions can be specified.
...
\end{equation}
where $\rho q_\mathrm{l}$ is the liquid water content. The flux is the sum $F=F_0+F_1$ of the solutions to the linear problems
\begin{subequations}
    \begin{align}
        &\partial_z F_0=\phantom{-}\kappa \rho q_\mathrm{l} F_0\;,\qquad F_0|_{z=z_\infty}=F_{c,0}\;,\\
        &\partial_z F_1=-\kappa \rho q_\mathrm{l} F_1\;,\qquad F_1|_{z=0}=F_{c,1}\;.
    \end{align}
\end{subequations}
