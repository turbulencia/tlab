...
%To compile, run {\tt ./configure} program, modify the {\tt Makefile} in the root directory if necessary, and run {\tt make}. By default two versions of the code will be generated: a release and a debug version.  The binaries will be located in {\tt ./bin/release} and {\tt ./bin/debug}. The libraries will be located in {\tt ./lib/release} and {\tt ./lib/debug}. To build just the release version, run {\tt make release}.

The root directory contains the sources for the common libraries, and the directory {\tt tools} contains the sources for the specific binaries: the main code in {\tt tools/dns}, and the preprocessing and postprocessing tools. %Each of the subdirectory in {\tt tools} generates one executable (at least).
