...
(incompressible mode). The symbol {\tt \#} is the number of the scalar.\\
\multicolumn{3}{l}{Similarly in the other directions {\tt I} and {\tt J}.}\\
...
\begin{longtable}{p{0.15\textwidth} p{0.3\textwidth} p{0.55\textwidth}}
  ...
  \multicolumn{3}{>{\columncolor{lightblue}}c}{\normalsize\bf [Filter]}\\
  % 
  \tt Type & \tt None, Compact, Explicit6, Explicit4, Adm, SpectralBand, SpectralErf, Alpha, TopHat & Type of domain filter used in simulation (if any) and postprocessing.\\
  \tt Parameters & {\em value1, value2, ...} & Values definining the filter, e.g., $\alpha$ in compact, or $\Delta$ in tophat.\\ 
  \tt ActiveX & \tt yes,no & Active or not in direction $Ox$.\\ 
\multicolumn{3}{l}{Similarly in the other directions $Oy$ and $Oz$.}\\
\end{longtable}
