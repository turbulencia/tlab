...
Spatial operators are based on finite difference methods (FDM). There are two levels of operator routines. The low-level libraries contains the basic algorithms and is explained in this section. It consists of the FDM kernel library {\tt fdm} and three-dimensional operators in the {\tt operators} library. (The latter is still part of the general {\tt dns} library but it should be migrated into its own {\tt operators} library. The file name starts, generally, with {\tt opr\_}.) The high-level library {\tt fields} is composed of routines that are just a combination of the low-level routines.
...
It is also interesting to note that one option to calculate the FD approximation to the second-order derivative is always $\delta_{x}\delta_{x} \mathbf{s} = (A_1^{-1}B_1)^2$, that is, to apply consecutively twice the FD approximation to the first-order derivative. However, the spectral transfer function of the FD operator $\delta_{x}\delta_{x}$ falls to zero at the Nyquist frequency, as shown in figure~\ref{fig:fdm}, which results in a very poor representation of the diffusion terms at the high wavenumbers. This property can become very important because the errors derived from the aliasing in calculating the non-linear terms accumulate and the use of filter might become unavoidable in order to have stable simulations. Hence, it is advisable to use a direct discretization of the second-order derivative operator.
...
There are global filters and directional filters. Global filters are 2D spectral filters in the horizontal plane, and Helmholtz-base filters. Directional filters are a sequence of one-dimensional filters in each direction. There are compact filters, which are implemented following \cite{Lele:1992}, and top-hat filters (or box filter), which are implemented using the trapezoidal rule to discretize the integral.
...
\begin{subequations}
  \begin{align}
    ...
    \end{align}
\end{subequations}
The non-periodic boundary conditions are implemented as
\begin{itemize}
\item biased, using biased schemes to define the filter.
\item free boundaries, using ghost cells where the function is the linear extrapolation of $s(x)$ based on the values at the two nodes next to the boundary.
\item solid boundaries, using ghost cells where the function is constant and equal to the boundary value of $s(x)$.
\item zero boundaries, where the filter size is monotonically reduced to zero at the boundary.
\end{itemize}
As an example, consider the solid boundary near $i=1$ for the example above:
\begin{subequations}
  \begin{align}
...
\begin{subequations}
  \begin{align}
    i=1,\ldots,n\;:\quad & i_\mathrm{org}=n-i+1\;,\\
    & \Delta_{f,i} = (i_\mathrm{org}+1)\Delta_{1}+\sum_{k=2}^{n+i-1}\Delta_{k} \;,\\
    &\tilde{c}_{i_\mathrm{org}+1,i} = (2\Delta_{f,i})^{-1}(2i_\mathrm{org}+1)\Delta_{1}\;,\\
...
\begin{subequations}
  \begin{align}
    i=i_\mathrm{max}-n+1,\ldots,i_\mathrm{max}\;:\quad & i_\mathrm{org}=n-i+1\;,\\
    & \Delta_{f,i} = \sum_{k=i-n}^{i_\mathrm{max}-2}\Delta_{k} +[(n+i-i_\mathrm{max})+1]\Delta_{i_\mathrm{max}-1}\;,\\
